...
int main() {
	debugf("test start\n");
        int fdnum = open("/test/testfile", O_RDWR);
        if (fdnum < 0) {
                user_panic("failed to open /test/testfile, return value: %d", fdnum);
        } else {
                debugf("finish opening /test/testfile\n");
        }
        int pid = fork();
        if (pid == 0) {
                debugf("child process\n");
                char buf[30];
                int len = read(fdnum, buf, 30);
                if (len < 0) {
                        user_panic("failed to get father file");
                } else {
			debugf("child successfully open the file by fdnum\n");
		}
                buf[len] = '\n';
                buf[len + 1] = 0;
                debugf("child get file content:\n");
                debugf(buf);
        } else {
                debugf("father process\n");
                char buf[30];
                int len = read(fdnum, buf, 30);
                if (len < 0) {
                        user_panic("failed to get father file");
                ...
                buf[len] = '\n';
                buf[len + 1] = 0;
                debugf("father get file content:\n");
		debugf(buf);
	}
	return 0;
	char test_str1[1 + 1];
