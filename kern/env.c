...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += BY2PG) {
		...
		page_insert(pgdir, asid, pa2page(pa + i), va + i, PTE_V|perm);
	}
}
...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	for (u_int i = 0; i < 64; i++) {
		e->sighandler_table[i].sa_handler = (void*)(e->sighandler_table[i].sa_mask.sig[0] = e->sighandler_table[i].sa_mask.sig[1] = 0);
	}
	e->env_sigprocmask.sig[0] = e->env_sigprocmask.sig[1] = 0;
...
static int load_icode_mapper(void *data, u_long va, size_t offset, u_int perm, const void *src,
	...
	if (page_alloc(&p)) {
		panic("[kern/env.c/load_icode_mapper]no more process space");
	}
	...
	if (src != NULL) {
		...
		memcpy((char*)page2kva(p) + offset, (char*)src, len);
	}
...
void env_free(struct Env *e) {
	...
	for (pdeno = 0; pdeno < PDX(UTOP); pdeno++) {
		...
		for (pteno = 0; pteno <= PTX(~0); pteno++) {
			if (pt[pteno] & PTE_V) {
				page_remove(e->env_pgdir, e->env_asid, (pdeno << PDSHIFT) | (pteno << PGSHIFT));
			}
		}
		...
		tlb_invalidate(e->env_asid, KSEG2 + (pdeno << PGSHIFT));
	}
	...
	tlb_invalidate(e->env_asid, KSEG2 + (PDX(UVPT) << PGSHIFT));
