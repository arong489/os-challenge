...
void schedule(int yield) {
	...
	if (curenv==NULL || yield || count==0 || e->env_status!=ENV_RUNNABLE) {
		if (curenv!=NULL) {
			TAILQ_REMOVE(&env_sched_list, e, env_sched_link);
		}
		if (curenv!=NULL && e->env_status==ENV_RUNNABLE) {
			TAILQ_INSERT_TAIL(&env_sched_list, e, env_sched_link);
		}
		if (TAILQ_EMPTY(&env_sched_list)) {
			panic("no runnale env");
		} else {
			e = TAILQ_FIRST(&env_sched_list);
			count = e->env_pri;
		}
//		while (e==NULL || count==0 || e->env_status==ENV_NOT_RUNNABLE) {
//			TAILQ_REMOVE(&env_sched_list, e, env_sched_link);
//			if (TAILQ_EMPTY(&env_sched_list)) {
//				panic("[kern/sched.c schedule:53]no runnable env");
//			} else {
//				e = TAILQ_FIRST(&env_sched_list);
//				count = e->env_pri;
//			}
