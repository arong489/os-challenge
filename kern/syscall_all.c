...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	if ((pp=page_lookup(srcenv->env_pgdir, srcva, NULL)) == 0) {
		return -E_INVAL;
	}
...
int sys_set_trapframe(u_int envid, struct Trapframe *tf) {
	...
	struct Env *env;
	try(envid2env(envid, &env, 1));
	// printk("env-0x%x sys_set_trapframe start epc%x\n", curenv->env_id, tf->cp0_epc);
	// print_tf(tf);
	if (env == curenv) {
...
int sys_back_do_signal(u_int envid, struct Trapframe *tf, sigset_t *originMask) {
	...
	int r;
	// printk("env-0x%x sys_back_do_signal start\n", curenv->env_id);
	if ((r = sys_set_trapframe(envid, tf)) < 0) {
		return r;
	}

	curenv->env_sigprocmask = *originMask;
	// printk("env-0x%x sys_back_do_signal to %x\n", curenv->env_id, tf->cp0_epc);
	return r;
}
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		if ((p=page_lookup(curenv->env_pgdir, srcva, NULL)) == 0) {
			return -E_INVAL;
		}
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_kill] = sys_kill,
	[SYS_back_do_signal] = sys_back_do_signal,
