...
struct Page_list page_free_list; /* miFree list of physical pages */
...
void page_init(void) {
	...
	// PPN(a) means a/4KiB
	// PADDR is virtual addr to physical addr
	int page_used_num = PPN(PADDR(freemem));
	for (size_t i = 0; i < page_used_num; i++) {
		pages[i].pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for (size_t i = page_used_num; i < npage; i++) {
		pages[i].pp_ref = 0;
...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	if (pte && (*pte & PTE_V)) {
		if (pa2page(*pte) != pp) {		//指向不同物理地址
			page_remove(pgdir, asid, va);	//
		} else {
			tlb_invalidate(asid, va);	//指向相同物理地址？？？？？
