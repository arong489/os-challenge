...
void syscall_sigaction(int, const struct sigaction*, struct sigaction*);
int syscall_kill(u_int, int);
void syscall_sigprocmask(int, const sigset_t*, sigset_t*);
...
void sigemptyset(sigset_t*); // 清空信号集，将所有位都设置为 0
void sigfillset(sigset_t*); // 设置信号集，即将所有位都设置为 1
void sigaddset(sigset_t*, int); // 向信号集中添加一个信号，即将指定信号的位设置为 1
void sigdelset(sigset_t*, int); // 从信号集中删除一个信号，即将指定信号的位设置为 0
int sigismember(const sigset_t*, int); // 检查一个信号是否在信号集中，如果在则返回 1，否则返回 0
