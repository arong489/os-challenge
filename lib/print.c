...
void vprintfmt(fmt_callback_t out, void *data, const char *fmt, va_list ap) {
	...
	for (;;) {
		...
		width = 0;
		while (*(fmt+width) && *(fmt+width) != '%' )width++;
		out(data, fmt, width);
		...
		while (*++fmt) {
			if (*fmt == '-') ladjust = 1;
			else if (*fmt == '+');/* todo:remain for future. '+' is stronger than ' ' */
			else if (*fmt == ' ');/* todo:remain for future */
			else if (*fmt == '0') padc = '0';
			else if (*fmt == '#');/* todo: %box egf, b&o&x is unsigned output*/
			else if (*fmt == '*');
			else break;
		}
		/* get width */
		...
		width = 0;
		while (*fmt && '0' <= *fmt && *fmt <= '9') width = width * 10 + *fmt++ - '0';
		/* check for long */
		/* Exercise 1.4: Your code here. (7/8) */
		long_flag = 0;
		if (*fmt == 'l') fmt++, long_flag = 1;
		neg_flag = 0;
		switch (*fmt) {
		case 'b':
			if (long_flag) {
